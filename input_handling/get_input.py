...
from input_handling.quit_game import quit_game
...
def input_number_of_players(human=True):
    if human:
        ...
        max_number = 7
    while True:
        num_players = input('How many {} players are there? '.format(player_type))
        try:
            ...
            if min_number <= num_players <= max_number:
                ...
                Enter a number from {0} to {1}.""".format(min_number, max_number, player_type))
        except ValueError:
            print('Please enter a numeral (such as 1 or 2).')


done_with_turn = {'finished', 'done'}
quit_game_cmds = {'quit', 'exit'}
...
for word in quit_game_cmds:
    possible_first_words.add(word)
