# The purpose of this module is to implement the help function, which players may call during their turn.
...
def print_move_command(selected_obj):
    ...
    print('ALSO, if you have already selected a unit or units, you may move them by the following:')
...
def print_set_command(selected_obj):
    print("The command 'set' is used to set the default build position of unit-producing buildings.")
    print("To use this command, you must first select an appropriate building (you have already built)")
    print("by using the 'select' command.")
    print("Once a building is selected, you can set its default build position by typing the following:")
    print("set default build position s3 e2")
    print('...where the direction is relative to the bottom left corner of the building. So the above '
          'command sets the default build position of the selected building to the position south 3 spots'
...
def help(selected_obj):
    ...
    while True:
        ...
        if inpt in help_functions:
            ...
            print('Sorry, either the command you requested information on does not exist, or the '
                  'help function does not yet include information on that command.')