...
assert p1.commands[NOW][BUILD_UNIT][towncenter] == ['villagers', 1]
assert p1.commands[LATER][BUILD_UNIT][towncenter] == ['villagers', 3]
...
assert p1.commands[NOW][BUILD_UNIT][barracks] == ['pikemen', 1]
assert p1.commands[LATER][BUILD_UNIT][barracks] == ['pikemen', 5]
...
for villager in (villager1, villager2):
    ...
    assert p1.commands[NOW][COLLECT_RESOURCE][villager] == Food
...
ls = p1.commands[NOW][BUILD_BUILDING][villager1]
house = ls[0]
assert isinstance(house, House)
position = ls[1]
assert position == Position(75, 75)
assert ls == p1.commands[LATER][BUILD_BUILDING][villager2]
...
assert ls == p1.commands[LATER][BUILD_BUILDING][villager3]
